...
function doPost(post_body) {
  ...
  // Chỗ này lấy post data từ client
  let { parameters, postData: { contents, type } = {} } = post_body;
  let response = {};

  // Nếu type ko support thì return
  if (
    type === "text/plain" ||
    type === "text/html" ||
  ...
  const activeSheetsAndNewParams = gidHandlerForPost(
    parameters,
    activeSpreadsheet,
    allSheets
  );
  ...
  if (type === "application/json" || (type === "" && contents.length > 0)) {
    ...
    jsonData = Array.isArray(jsonData)
      ? jsonData.map((data) => flatten(data))
      : [flatten(jsonData)];
    keys = Array.isArray(jsonData)
      ? jsonData[0].constructor === Object || jsonData[0].constructor === Array
    ...
    if (keys.length > 0) {
      activeSheets.forEach((activeSheetName) => {
        ...
        if (
          headers.length == 0 ||
          (headers.length == 1 && headers[0].length == 0)
        ) {
        ...
        jsonData.forEach((rowLevelData) =>
          [rowLevelData].map((row) =>
            rowData.push(
              headers.map((key) =>
                key === "timestamp_incoming_webhook"
                  ? now
                  : row[String(key)] || ""
              )
            )
    ...
    if (parameters) {
      ...
      if (keys.length > 0) {
        logTimeStamp === true
          ? (parameters["timestamp_incoming_webhook"] = [new Date()])
          : null;
        keys = Object.keys(parameters);
        const cartesianData = cartesian(parameters);
        activeSheets.forEach((activeSheetName) => {
          let activeSheet = activeSpreadsheet.getSheetByName(activeSheetName);
          let headers = activeSheet
            .getDataRange()
            .offset(0, 0, 1)
            .getValues()[0];
          if (
            ...
            activeSheet.setFrozenRows(1);
            if (logTimeStamp === true) {
              activeSheet.moveColumns(
                activeSheet.getRange(
                  1,
                  keys.indexOf("timestamp_incoming_webhook") + 1
                ),
                1
              );
              SpreadsheetApp.flush();
...
function gidHandlerForPost(params, activeSpreadsheet, allSheets) {
  ...
  allSheets.forEach((sheet) =>
    existingSheetIds.push(sheet.getSheetId().toString())
  );

  let defaultWebhookPostSheetId = documentProperties.getProperty(
    "defaultWebhookPostSheetId"
  );
  let newDefaultWebhookPostSheetName = `[POST] Webhook — ${new Date()
    .getTime()
    .toString()}`;
  ...
  if (keys.includes("gid")) {
    ...
    const matchingGids = existingSheetIds.filter((sheetId) =>
      gidValues.includes(sheetId)
    );
    const nonMatchingGids = gidValues.filter(
      (gid) => !matchingGids.includes(gid)
    );
    if (matchingGids.length === 0) {
      ...
      if (matchingGids.length === 1) {
        postDefaultSheet = allSheets.filter(
          (sheet) => sheet.getSheetId() == matchingGids[0]
        );
        return {
  ...
  if (checkDefaultOrCreateNewPostSheet) {
    if (!defaultWebhookPostSheetId) {
      defaultWebhookPostSheetId = activeSpreadsheet
        .insertSheet()
        .setName(newDefaultWebhookPostSheetName)
        .getSheetId()
        .toString();
      ...
      postDefaultSheet = allSheets.filter(
        (sheet) => sheet.getSheetId() == defaultWebhookPostSheetId
      );
      if (postDefaultSheet.length !== 0) {
        return {
          activeSheetNames: [postDefaultSheet[0].getSheetName()],
